...
namespace implemented
{
    class Camera
    {
     public:
        ...
        {
            // Move the camera using the up vector
            position += glm::normalize(up) * distance;

            // Prevent the camera from moving below the XZ plane
            if (position.y < 0.8f) {
                position.y = 0.8f; // Keep the camera above the XZ plane
            }

            glm::vec3 targetPosition = GetTargetPosition();
            // Prevent the target from moving below the XZ plane
            if (targetPosition.y < 0.8f) {
                targetPosition.y = 0.8f; // Keep the target above the XZ plane
            }

            // Adjust the camera's forward vector based on the new target position
